...
void OS_Application_Startup(void)
{
    ...
    */
    ...
    /*
    ** Set up the virtual FS mappings for the pisat deploy layout.
    **
    ** Trust-split filesystem (physical dirs must be pre-created by the deploy;
    ** these targets MUST match the SPACECOP_*_DIR macros in
    ** apps/spacecop/platform_inc/spacecop_platform_cfg.h):
    **   /cf   -> /opt/pisat/cf   read-only code/tables/kernel-module + config
    **   /data -> /var/pisat/data writable pictures
    **   /logs -> /var/pisat/logs writable IDS + app logs
    **   /cti  -> /var/pisat/cti  writable threat-intel sharing
    **
    ** /data,/logs,/cti should be a separate noexec,nosuid,nodev mount from /cf.
    */
    Status = OS_FileSysAddFixedMap(&fs_id, "/opt/pisat/cf", "/cf");
    if (Status != OS_SUCCESS)
    {
        /* Print for informational purposes --
         * startup can continue, but loads may fail later, depending on config. */
        OS_printf("CFE_PSP: OS_FileSysAddFixedMap(/cf) failure: %d\n", (int)Status);
    }

    Status = OS_FileSysAddFixedMap(&fs_id, "/var/pisat/data", "/data");
    if (Status != OS_SUCCESS)
    {
        OS_printf("CFE_PSP: OS_FileSysAddFixedMap(/data) failure: %d\n", (int)Status);
    }

    Status = OS_FileSysAddFixedMap(&fs_id, "/var/pisat/logs", "/logs");
    if (Status != OS_SUCCESS)
    {
        OS_printf("CFE_PSP: OS_FileSysAddFixedMap(/logs) failure: %d\n", (int)Status);
    }

    Status = OS_FileSysAddFixedMap(&fs_id, "/var/pisat/cti", "/cti");
    if (Status != OS_SUCCESS)
    {
        OS_printf("CFE_PSP: OS_FileSysAddFixedMap(/cti) failure: %d\n", (int)Status);
    }
