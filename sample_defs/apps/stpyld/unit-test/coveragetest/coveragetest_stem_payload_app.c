...
/*
** File: coveragetest_stem_payload_app.c
**
** Purpose:
** Coverage Unit Test cases for the STEM_PAYLOAD Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the STEM_PAYLOAD application.
**
...
#include "stem_payload_app_coveragetest_common.h"
#include "ut_stem_payload_app.h"

/* to get the STEM_PAYLOAD_LIB_Function() declaration */
...
void Test_STEM_PAYLOAD_AppMain(void)
{
    CFE_SB_MsgId_t MsgId = CFE_SB_INVALID_MSG_ID;

    /*
     * Test Case For:
     * void STEM_PAYLOAD_AppMain( void )
     */

    UT_CheckEvent_t EventTest;

    /*
     * STEM_PAYLOAD_AppMain does not return a value,
     ...
     * The first is for STEM_PAYLOAD_AppInit().  As this is in the same
     ...
     * the STEM_PAYLOAD_AppInit() failure path was taken.
     */
    STEM_PAYLOAD_AppMain();
     ...
     */
    UtAssert_True(STEM_PAYLOAD_AppData.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "STEM_PAYLOAD_AppData.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)STEM_PAYLOAD_AppData.RunStatus);
    ...
    UT_CheckEvent_Setup(&EventTest, STEM_PAYLOAD_PIPE_ERR_EID, "STEM_PAYLOAD: SB Pipe Read Error = %d");
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "STEM_PAYLOAD_PIPE_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_STEM_PAYLOAD_AppInit(void)
{
    /*
     * Test Case For:
     * int32 STEM_PAYLOAD_AppInit( void )
     */

    /* nominal case should return CFE_SUCCESS */
    UT_TEST_FUNCTION_RC(STEM_PAYLOAD_AppInit(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UT_TEST_FUNCTION_RC(STEM_PAYLOAD_AppInit(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "CFE_ES_WriteToSysLog() called");

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UT_TEST_FUNCTION_RC(STEM_PAYLOAD_AppInit(), CFE_SB_BAD_ARGUMENT);
...
void Test_STEM_PAYLOAD_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void STEM_PAYLOAD_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        STEM_PAYLOAD_NoArgs_cmd_t  Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, STEM_PAYLOAD_PROCESS_CMD_ERR_EID, NULL);

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(STEM_PAYLOAD_CMD_MID);
    FcnCode   = STEM_PAYLOAD_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    STEM_PAYLOAD_ProcessCommandPacket();
    UtAssert_True(EventTest.MatchCount == 0, "STEM_PAYLOAD_CMD_ERR_EID not generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    STEM_PAYLOAD_ProcessCommandPacket();
    UtAssert_True(EventTest.MatchCount == 1, "STEM_PAYLOAD_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_STEM_PAYLOAD_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void STEM_PAYLOAD_ProcessGroundCommand
     */
    CFE_SB_MsgId_t    TestMsgId = CFE_SB_ValueToMsgId(STEM_PAYLOAD_CMD_MID);
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t           SBBuf;
        STEM_PAYLOAD_NoArgs_cmd_t       Noop;
        STEM_PAYLOAD_NoArgs_cmd_t       Reset;
    } TestMsg;
    UT_CheckEvent_t EventTest;
    ...
    UT_CheckEvent_Setup(&EventTest, STEM_PAYLOAD_CMD_NOOP_INF_EID, NULL);
    STEM_PAYLOAD_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "STEM_PAYLOAD_CMD_NOOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of RESET */
    FcnCode = STEM_PAYLOAD_RESET_COUNTERS_CC;
    Size    = sizeof(TestMsg.Reset);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &Size, sizeof(Size), false);
    UT_CheckEvent_Setup(&EventTest, STEM_PAYLOAD_CMD_RESET_INF_EID, NULL);
    STEM_PAYLOAD_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "STEM_PAYLOAD_CMD_RESET_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CheckEvent_Setup(&EventTest, STEM_PAYLOAD_CMD_ERR_EID, NULL);
    STEM_PAYLOAD_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "STEM_PAYLOAD_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_STEM_PAYLOAD_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void STEM_PAYLOAD_ReportHousekeeping()
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(STEM_PAYLOAD_REQ_HK_TLM);

    /* Set message id to return so STEM_PAYLOAD_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    STEM_PAYLOAD_ReportHousekeeping();

    /* Confirm message sent*/
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TransmitMsg)) == 1, "CFE_SB_TransmitMsg() called once");
    UtAssert_True(MsgSend == &STEM_PAYLOAD_AppData.HkTelemetryPkt.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &STEM_PAYLOAD_AppData.HkTelemetryPkt.TlmHeader.Msg,
                  "CFE_SB_TimeStampMsg() address matches expected");
}

void Test_STEM_PAYLOAD_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool STEM_PAYLOAD_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    CFE_MSG_FcnCode_t fcncode = 2;
    CFE_SB_MsgId_t    msgid   = CFE_SB_ValueToMsgId(STEM_PAYLOAD_CMD_MID);

    /*
     * test a match case
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, STEM_PAYLOAD_LEN_ERR_EID, NULL);

    STEM_PAYLOAD_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "STEM_PAYLOAD_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    UT_CheckEvent_Setup(&EventTest, STEM_PAYLOAD_LEN_ERR_EID, NULL);
    STEM_PAYLOAD_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "STEM_PAYLOAD_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
 ...
 */
void Stem_payload_UT_Setup(void)
{
    UT_ResetState(0);
}
 ...
 */
void Stem_payload_UT_TearDown(void) {}
...
{
    ADD_TEST(STEM_PAYLOAD_AppMain);
    ADD_TEST(STEM_PAYLOAD_AppInit);
    ADD_TEST(STEM_PAYLOAD_ProcessCommandPacket);
    ADD_TEST(STEM_PAYLOAD_ProcessGroundCommand);
    ADD_TEST(STEM_PAYLOAD_ReportHousekeeping);
    ADD_TEST(STEM_PAYLOAD_VerifyCmdLength);
}
