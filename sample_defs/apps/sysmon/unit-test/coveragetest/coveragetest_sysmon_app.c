...
/*
** File: coveragetest_sysmon_app.c
**
** Purpose:
** Coverage Unit Test cases for the SYSMON Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the SYSMON application.
**
...
#include "sysmon_app_coveragetest_common.h"
#include "ut_sysmon_app.h"

/* to get the SYSMON_LIB_Function() declaration */
...
void Test_SYSMON_AppMain(void)
{
    ...
    /*
     * SYSMON_AppMain does not return a value,
     ...
     * The first is for SYSMON_AppInit().  As this is in the same
     ...
     * the SYSMON_AppInit() failure path was taken.
     */
    SYSMON_AppMain();
     ...
     */
    UtAssert_True(SYSMON_AppData.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "SYSMON_AppData.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)SYSMON_AppData.RunStatus);
    ...
    UT_CheckEvent_Setup(&EventTest, SYSMON_PIPE_ERR_EID, "SYSMON: SB Pipe Read Error = %d");
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "SYSMON_PIPE_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_SYSMON_AppInit(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(SYSMON_AppInit(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UT_TEST_FUNCTION_RC(SYSMON_AppInit(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "CFE_ES_WriteToSysLog() called");

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UT_TEST_FUNCTION_RC(SYSMON_AppInit(), CFE_SB_BAD_ARGUMENT);
...
void Test_SYSMON_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void SYSMON_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        SYSMON_NoArgs_cmd_t  Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, SYSMON_PROCESS_CMD_ERR_EID, NULL);

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(SYSMON_CMD_MID);
    FcnCode   = SYSMON_NOOP_CC;
    ...
    UtAssert_True(EventTest.MatchCount == 0, "SYSMON_CMD_ERR_EID not generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    SYSMON_ProcessCommandPacket();
    UtAssert_True(EventTest.MatchCount == 1, "SYSMON_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_SYSMON_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void SYSMON_ProcessGroundCommand
     */
    CFE_SB_MsgId_t    TestMsgId = CFE_SB_ValueToMsgId(SYSMON_CMD_MID);
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    UT_CheckEvent_Setup(&EventTest, SYSMON_CMD_NOOP_INF_EID, NULL);
    SYSMON_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "SYSMON_CMD_NOOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of RESET */
    FcnCode = SYSMON_RESET_COUNTERS_CC;
    Size    = sizeof(TestMsg.Reset);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &Size, sizeof(Size), false);
    UT_CheckEvent_Setup(&EventTest, SYSMON_CMD_RESET_INF_EID, NULL);
    SYSMON_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "SYSMON_CMD_RESET_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CheckEvent_Setup(&EventTest, SYSMON_CMD_ERR_EID, NULL);
    SYSMON_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "SYSMON_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_SYSMON_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void SYSMON_ReportHousekeeping()
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(SYSMON_REQ_HK_TLM);

    /* Set message id to return so SYSMON_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    UtAssert_True(MsgSend == &SYSMON_AppData.HkTelemetryPkt.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &SYSMON_AppData.HkTelemetryPkt.TlmHeader.Msg,
                  "CFE_SB_TimeStampMsg() address matches expected");
}

void Test_SYSMON_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool SYSMON_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    CFE_MSG_FcnCode_t fcncode = 2;
    CFE_SB_MsgId_t    msgid   = CFE_SB_ValueToMsgId(SYSMON_CMD_MID);

    /*
     * test a match case
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, SYSMON_LEN_ERR_EID, NULL);

    SYSMON_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "SYSMON_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    UT_CheckEvent_Setup(&EventTest, SYSMON_LEN_ERR_EID, NULL);
    SYSMON_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "SYSMON_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
...
{
    ADD_TEST(SYSMON_AppMain);
    ADD_TEST(SYSMON_AppInit);
    ADD_TEST(SYSMON_ProcessCommandPacket);
    ADD_TEST(SYSMON_ProcessGroundCommand);
    ADD_TEST(SYSMON_ReportHousekeeping);
    ADD_TEST(SYSMON_VerifyCmdLength);
}
