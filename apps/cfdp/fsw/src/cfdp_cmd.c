...
void CFDP_UploadToSatellite(const CFE_MSG_Message_t *MsgPtr)
{
    ...
    CFDP_CFDP_GetFile(CmdPtr->pdu_type, CmdPtr->buffer, CmdPtr->destination, CmdPtr->length,
                      CmdPtr->expected_size, CmdPtr->expected_crc);

}

void CFDP_ResendChunk(const CFE_MSG_Message_t *MsgPtr)
{
    const CFDP_ResendChunk_t *CmdPtr = (const CFDP_ResendChunk_t *)MsgPtr;

    CFDP_CFDP_ResendChunk(CmdPtr->source, CmdPtr->destination, CmdPtr->seq_num);
}