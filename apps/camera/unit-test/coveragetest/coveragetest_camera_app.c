...
/*
** File: coveragetest_camera_app.c
**
** Purpose:
** Coverage Unit Test cases for the CAMERA Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the CAMERA application.
**
...
#include "camera_app_coveragetest_common.h"
#include "ut_camera_app.h"

/* to get the CAMERA_LIB_Function() declaration */
...
void Test_CAMERA_AppMain(void)
{
    ...
    /*
     * CAMERA_AppMain does not return a value,
     ...
     * The first is for CAMERA_AppInit().  As this is in the same
     ...
     * the CAMERA_AppInit() failure path was taken.
     */
    CAMERA_AppMain();
     ...
     */
    UtAssert_True(CAMERA_AppData.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "CAMERA_AppData.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)CAMERA_AppData.RunStatus);
    ...
    UT_CheckEvent_Setup(&EventTest, CAMERA_PIPE_ERR_EID, "CAMERA: SB Pipe Read Error = %d");
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "CAMERA_PIPE_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_CAMERA_AppInit(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(CAMERA_AppInit(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UT_TEST_FUNCTION_RC(CAMERA_AppInit(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "CFE_ES_WriteToSysLog() called");

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UT_TEST_FUNCTION_RC(CAMERA_AppInit(), CFE_SB_BAD_ARGUMENT);
...
void Test_CAMERA_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void CAMERA_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        CAMERA_NoArgs_cmd_t  Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, CAMERA_PROCESS_CMD_ERR_EID, NULL);

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(CAMERA_CMD_MID);
    FcnCode   = CAMERA_NOOP_CC;
    ...
    UtAssert_True(EventTest.MatchCount == 0, "CAMERA_CMD_ERR_EID not generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    CAMERA_ProcessCommandPacket();
    UtAssert_True(EventTest.MatchCount == 1, "CAMERA_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_CAMERA_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void CAMERA_ProcessGroundCommand
     */
    CFE_SB_MsgId_t    TestMsgId = CFE_SB_ValueToMsgId(CAMERA_CMD_MID);
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    UT_CheckEvent_Setup(&EventTest, CAMERA_CMD_NOOP_INF_EID, NULL);
    CAMERA_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "CAMERA_CMD_NOOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of RESET */
    FcnCode = CAMERA_RESET_COUNTERS_CC;
    Size    = sizeof(TestMsg.Reset);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &Size, sizeof(Size), false);
    UT_CheckEvent_Setup(&EventTest, CAMERA_CMD_RESET_INF_EID, NULL);
    CAMERA_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "CAMERA_CMD_RESET_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CheckEvent_Setup(&EventTest, CAMERA_CMD_ERR_EID, NULL);
    CAMERA_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "CAMERA_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_CAMERA_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void CAMERA_ReportHousekeeping()
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(CAMERA_REQ_HK_TLM);

    /* Set message id to return so CAMERA_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    UtAssert_True(MsgSend == &CAMERA_AppData.HkTelemetryPkt.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &CAMERA_AppData.HkTelemetryPkt.TlmHeader.Msg,
                  "CFE_SB_TimeStampMsg() address matches expected");
}

void Test_CAMERA_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool CAMERA_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    CFE_MSG_FcnCode_t fcncode = 2;
    CFE_SB_MsgId_t    msgid   = CFE_SB_ValueToMsgId(CAMERA_CMD_MID);

    /*
     * test a match case
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, CAMERA_LEN_ERR_EID, NULL);

    CAMERA_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "CAMERA_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    UT_CheckEvent_Setup(&EventTest, CAMERA_LEN_ERR_EID, NULL);
    CAMERA_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "CAMERA_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
...
{
    ADD_TEST(CAMERA_AppMain);
    ADD_TEST(CAMERA_AppInit);
    ADD_TEST(CAMERA_ProcessCommandPacket);
    ADD_TEST(CAMERA_ProcessGroundCommand);
    ADD_TEST(CAMERA_ReportHousekeeping);
    ADD_TEST(CAMERA_VerifyCmdLength);
}
